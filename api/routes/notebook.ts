...
app.post('/', async (c) => {
  ...
  const now = new Date()
  const result = await db.insert(notebook).values({ title, createDate: now, updatedDate: now } as any)
...
app.put('/:id', async (c) => {
  ...
  await db.update(notebook).set({ title, updatedDate: new Date() }).where(eq(notebook.id, id))
...
app.get('/:id/pages', async (c) => {
  ...
  const pages = await db.select().from(page).where(eq(page.notebookId, notebookId))
  return c.json(pages)
})
...
app.post('/:id/pages', async (c) => {
  ...
  const now = new Date()
  await db.insert(page).values({ notebookId, title, content, createDate: now, updatedDate: now } as any)
