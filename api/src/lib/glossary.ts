...
export async function maybeRunGlossaryUpdates(
  ...
  for (const ch of changes) {
    try {
      // Only process if there was a change
      if (!ch.changed) continue;

      console.log("Checking glossary entry for termId:", ch.termId);
      const [entry] = await db
        .select()
        .from(glossaryEntry)
        .where(
          eq(glossaryEntry.termId, ch.termId),
          eq(glossaryEntry.notebookId, ch.notebookId),
        );
    ...
    if (!existing) {
      await db
        ...
        .values({
          notebookId,
          termId,
          sourceHash: excerptsHash,
          content: summary,
        } as any);
    } else {
      await db
        .update(glossaryEntry)
        .set({ sourceHash: excerptsHash, content: summary })
